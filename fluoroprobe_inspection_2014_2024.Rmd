...
flora_heatmap <- function(fp_data, reservoir, year, site, z){
  ...
  fp_new$Date <- as.Date(fp_new$DateTime)
  
  fig_title <- paste(reservoir, year, "Site", site, z, sep = " ")
  
  # Interpolation with dates
  interp <- interp(
    x = as.numeric(fp_new$Date),  # convert Date to numeric for interp()
    y = fp_new$Depth_m,
    z = unlist(fp_new[z]),
    xo = seq(min(as.numeric(fp_new$Date)), max(as.numeric(fp_new$Date)), by = 1),
  ...
  interp$x <- as.Date(interp$x, origin = "1970-01-01")  # convert back to Date
  ...
  p1 <- ggplot(interp, aes(x = x, y = y)) +
    ...
    labs(x = "Month", y = "Depth (m)", title = fig_title,
         fill = expression(paste(mu, g/L))) +
    theme_bw()
